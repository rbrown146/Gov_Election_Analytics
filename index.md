...
Democratic voter turnout (negative is higher) = **-2.137 - 0.024** x avg. Dem. poll support + vector of state fixed effects + vector of state offsets to avg. Dem. poll suport + **0.117** x % Hispanic population + **0.202** x % Indigenous population **- 0.885** x % Asian population + **0.056** x % Black population **- 0.007** x % population aged 30-45 + **0.035** x population aged 20-30 + **0.008** x population aged 65 and older **-0.073** x percentage change in population aged 30-45 over past 4 years + **0.036** x percentage change in population that is Hispanic **- 0.068** x percentage change in population aged 65 and older **- 0.074** x percentage change in population aged 20-30 + **0.201** x percentage change in population that is Black **- 0.073** x percentage change in population that is Asian **-0.034** x percentage change in population that is Indigenous + vector of regional offsets to percentage change in each race + vector of regional offsets to percentage change in each age group

The model for Republican voter turnout has the same format except that it uses Republican poll support. In this case, positive numbers are indicative of greater support for the GOP.

What do the coefficients on each of these terms mean? They tell you

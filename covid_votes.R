...
for (s in state_list) {
  ...
  state_forecast[[s]]$mod_D <- glm(cbind(avg_poll, 100-avg_poll) ~ covid_pct + D_pv2p, 
  ...
  state_forecast[[s]]$mod_R <- glm(cbind(avg_poll, 100-avg_poll) ~ covid_pct + R_pv2p, 
  ...
  state_forecast[[s]]$prob_Rvote <- predict(state_forecast[[s]]$mod_R, newdata = data.frame(covid_pct=covid_polls$covid_pct[[1]], R_pv2p=biden_raw_vote_old$R_pv2p[[1]]), se=T, type="response")
  state_forecast[[s]]$prob_Dvote <- predict(state_forecast[[s]]$mod_D, newdata = data.frame(covid_pct=covid_polls$covid_pct[[1]], D_pv2p=trump_raw_vote_old$D_pv2p[[1]]), se=T, type="response")
...
plot_usmap(data = state_means, regions = "states", values = "value2") +
  ...
  labs(title="State Winner Based on Covid Data and 2016 Outcome") +
  theme_void()

ggsave("covid_2016.jpg")

# Create df to examine polling averages without covid data
polls_plain <- polls_2020 %>%
  group_by(state) %>%
  mutate(diff = diff(avg_poll), pv2p = avg_poll/sum(avg_poll)) %>%
  select(-answer) %>%
  filter(!state %in% c("", "Nebraska CD-1", "Nebraska CD-2", "Maine CD-1", "Maine CD-2")) %>%
  mutate(winner = ifelse(diff > 0,"Republican","Democrat"))

# Plot map of polling averages without covid data
plot_usmap(data = polls_plain, regions = "states", values = "winner") +
  scale_fill_manual(values = c("blue", "red"), name = "state PV winner") +
  labs(title="State Winner Based on Polls (No Covid Data)") +
  theme(legend.position = "right")

ggsave("polls_no_covid.jpg")

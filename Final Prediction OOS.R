...
missing_states <- c("Wyoming", "North Dakota", "South Dakota", "Nebraska", "Illinois", "West Virginia", "Rhode Island")
...
for(s in missing_states){
  ...
  missing[[s]]$Rpoll <- sum(missing[[s]][["Rvote"]][["fit"]])/40
  missing[[s]]$Dpoll <- sum(missing[[s]][["Dvote"]][["fit"]])/40
...
plot_usmap(data = state_means, regions = "states", values = "win_test") +
  scale_fill_manual(values = c("blue", "red"), name = "State PV Winner, Test Prediction (2000)") +
  theme_void()
...
gore_ci <- c(d_00 - 1.96*pop_vote_test$d_sd_test, d_00 + 1.96*pop_vote_test$d_sd_test)
gore_ci

# Get Bush confidence interval
bush_ci <- c(r_00 - 1.96*pop_vote_test$r_sd_test, r_00 + 1.96*pop_vote_test$r_sd_test)
bush_ci

...
for (s in state_list) {
  ...
  state_forecast[[s]]$prob_Rvote <- predict(state_forecast[[s]]$mod_R, newdata = data.frame(avg_poll=trump_raw_vote[[3]]), se=T, type="response")
  state_forecast[[s]]$prob_Dvote <- predict(state_forecast[[s]]$mod_D, newdata = data.frame(avg_poll=biden_raw_vote[[3]]), se=T, type="response")

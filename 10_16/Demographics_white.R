...
for (s in state_list) {
  ...
  state_forecast[[s]]$mod_D <- glm(cbind(D, VEP-D) ~ avg_poll + Female + White, 
  ...
  state_forecast[[s]]$mod_R <- glm(cbind(R, VEP-R) ~ avg_poll + Female + White, 
  ...
  state_forecast[[s]]$prob_Rvote <- predict(state_forecast[[s]]$mod_R, newdata = data.frame(avg_poll=trump_raw_vote[[3]], Female=demograph_2018$Female[[1]], White=demograph_2018$White[[1]]), se=T, type="response")
  state_forecast[[s]]$prob_Dvote <- predict(state_forecast[[s]]$mod_D, newdata = data.frame(avg_poll=biden_raw_vote[[3]], Female=demograph_2018$Female[[1]], White=demograph_2018$White[[1]]), se=T, type="response")
